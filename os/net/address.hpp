...
#ifndef DMITIGR_OS_NET_ADDRESS_HPP
#define DMITIGR_OS_NET_ADDRESS_HPP
...
private:
  ...
  static int inet_pton__(const int af, const char* const src, void* const dst)
  {
    ...
    if (result < 0)
      throw DMITIGR_OS_NET_EXCEPTION{"cannot convert text "
  ...
  static void inet_ntop__(const int af, const void* const src, char* const dst, const unsigned dst_size)
  {
    ...
    if (!::inet_ntop(af, src, dst, dst_size))
      throw DMITIGR_OS_NET_EXCEPTION{"cannot convert numeric "
...
#endif  // DMITIGR_OS_NET_ADDRESS_HPP
