...
#ifndef DMITIGR_OS_NET_LISTENER_HPP
#define DMITIGR_OS_NET_LISTENER_HPP
...
public:
  ...
  void listen() override
  {
    ...
    const auto tcp_create_bind = [&]
    {
      ...
      if (::setsockopt(socket_, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&optval), optlen) != 0)
        throw DMITIGR_OS_NET_EXCEPTION{"cannot set SO_REUSEADDR socket option"};
    ...
    if (::listen(socket_, *options_.backlog()) != 0)
      throw DMITIGR_OS_NET_EXCEPTION{"cannot start listening on socket"};
  }
  ...
  std::unique_ptr<Descriptor> accept() override
  {
    ...
    if (net::Socket_guard sock{::accept(socket_, addr, addrlen)}; !net::is_socket_valid(sock))
      throw DMITIGR_OS_NET_EXCEPTION{"cannot accept on socket"};
    else
  ...
  {
    if (socket_.close() != 0)
      throw DMITIGR_OS_NET_EXCEPTION{"cannot close socket"};
  }
...
#endif  // DMITIGR_OS_NET_LISTENER_HPP
