...
#ifndef DMITIGR_OS_NET_DESCRIPTOR_HPP
#define DMITIGR_OS_NET_DESCRIPTOR_HPP
...
public:
  ...
  std::streamsize read(char* const buf, std::streamsize len) override
  {
    ...
    if (net::is_socket_error(result))
      throw DMITIGR_OS_NET_EXCEPTION{"cannot read from socket"};
  ...
  std::streamsize write(const char* const buf, std::streamsize len) override
  {
    ...
    if (net::is_socket_error(result))
      throw DMITIGR_OS_NET_EXCEPTION{"cannot write to socket"};
  ...
  void close() override
  {
    ...
    if (socket_.close() != 0)
      throw Sys_exception{"cannot close socket"};
  }
  ...
  void graceful_shutdown()
  {
    ...
    if (const auto r = ::shutdown(socket_, net::sd_send)) {
      if (errno == ENOTCONN)
        ...
        throw DMITIGR_OS_NET_EXCEPTION{errmsg};
    }
    while (true) {
...
#endif  // DMITIGR_OS_NET_DESCRIPTOR_HPP
