...
public:
  ...
  {
    DMINT_REQUIRE(positional_parameter_count() <= index && index < parameter_count());
  ...
  {
    DMINT_REQUIRE(index < parameter_count());
...
private:
  ...
  {
    DMINT_REQUIRE(!data || data->size() <= maximum_data_size());
    if (!is_preparsed() && !is_described()) {
      DMINT_REQUIRE(index < maximum_parameter_count());
