...
void pq_Prepared_statement::init_connection__(pq_Connection* const connection)
{
  ...
  DMITIGR_PGFE_INTERNAL_ASSERT(connection && connection->session_start_time());
...
{
  DMITIGR_PGFE_INTERNAL_REQUIRE(connection()->is_ready_for_async_request());
...
{
  DMITIGR_PGFE_INTERNAL_REQUIRE(connection()->is_ready_for_request());
  execute_async();
